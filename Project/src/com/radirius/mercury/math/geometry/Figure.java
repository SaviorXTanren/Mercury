...
public class Figure {
	...
	/**
	 * @param vertices
	 * 		All vertices in the figure.
	 */
	protected Figure(Vector2f... vertices) {
	 ...
	 * 		The coordinates of all vertices in the figure. Will be parsed for every 2 values into a Vector2f.
	 */
	 ...
	 *
	 * @param coordinates
	 * 		The x/y pattern of floats
	 *
	 * @return an array of 2 dimensional vectors based on coordinates
	 */
	protected static Vector2f[] getVectors(float... coordinates) {
		...
		for (int vertexIndex = 0; vertexIndex < coordinates.length; vertexIndex += 2)
			vectors[vertexIndex / 2] = new Vector2f(coordinates[vertexIndex], coordinates[vertexIndex + 1]);
	...
	public Point[] getIntersectionPoints(Figure figure) {
		...
		for (int vertex0 = 0; vertex0 < vertices.length; ) {
			...
			Vector2f line1Vertex1 = vertices[vertex0], line1Vertex2 = vertices.length > 2 ? vertices[++vertex0 % vertices.length] : vertices[++vertex0];
			...
			for (int vertex1 = 0; vertex1 < figure.vertices.length; ) {
				...
				Vector2f line2Vertex1 = figure.vertices[vertex1], line2Vertex2 = figure.vertices.length > 2 ? figure.vertices[++vertex1 % figure.vertices.length] : figure.vertices[++vertex1];
	...
	public boolean contains(Vector2f vertex) {
		...
		for (Vector2f vertex_ : vertices) {
			float deltaX = vertex.x - vertex_.x, deltaY = vertex.y - vertex_.x;
	 ...
	 *
	 * @param x
	 * 		Where every vertex should move relative to the nearest point of the figure on x.
	 * @param y
	 * 		Where every vertex should move relative to the nearest point of the figure on y.
	 *
	 * @return the figure.
	 */
	 ...
	 * 		The origin's y.
	 * @param angle
	 * 		The angle by which the object will rotate relative to the origin.
	 *
	 * @return the figure.
	 */
	public Figure rotate(float originX, float originY, float angle) {
		if (angle == 0 || Math.round(angle * 100) == (int) Math.round(Math.PI * 100))
			return this;
	 ...
	 * 		The origin's y.
	 * @param angle
	 * 		The angle by which the object will rotate to relative to the origin.
	 *
	 * @return the figure.
	 */
	 ...
	 * @return the figure.
	 */
	public Figure dilate(Vector2f point, float dilate) {
	 ...
	 * @return the figure.
	 */
	public Figure setDilation(Vector2f point, float dilate) {

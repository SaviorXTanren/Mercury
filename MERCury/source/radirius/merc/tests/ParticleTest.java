...
public class ParticleTest extends Core {
    ...
    public void init() {
        ...
        emitter = new ParticleEmitter((Rectangle) new Rectangle(bullet.getX(), bullet.getY(), bullet.getWidth(),
                bullet.getHeight()).scale(0.4f), pes);
        bullet.addChild(emitter.getEmitterBounds());
    }
    
    @Override
    public void update(float delta) {
        bullet.translate(Math.abs(vel) < 10 ? vel *= 1.01f : vel, 0).rotate(vel / 2)
                .setScale(1f + (float) (0.2f * Math.sin(rnr.getTime() / 50)));
        emitter.getOptions().speed = -vel * 0.02f;
        emitter.getOptions().size = (float) Math.min(Math.abs(vel), 16);

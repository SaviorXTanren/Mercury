...
public class Window extends Panel {
    ...
    public Window(String name, Rectangle bounds, float padding) {
        this(name, TrueTypeFont.OPENSANS_REGULAR, Color.DEFAULT_TEXT_COLOR, bounds, padding, getDefaultTextures()
                .getTexture(1), getDefaultTextures().getTexture(2), getDefaultTextures().getTexture(0),
                new ImageButton(new Rectangle(0, 0, 32, 32)));
    }
    
    public Window(String name, Rectangle bounds, float padding, Texture body, Texture border, Texture bar,
            ImageButton exit) {
        this(name, TrueTypeFont.OPENSANS_REGULAR, Color.DEFAULT_TEXT_COLOR, bounds, padding, body, border, bar
                .getHeight(), bar, exit);
    }
    ...
    public Window(String name, Color namecol, Rectangle bounds, float padding) {
        this(name, TrueTypeFont.OPENSANS_REGULAR, namecol, bounds, padding, getDefaultTextures().getTexture(1)
                .convertToTexture(), getDefaultTextures().getTexture(2), getDefaultTextures().getTexture(0),
                new ImageButton(new Rectangle(0, 0, 32, 32)));
    }
    
    public Window(String name, Color namecol, Rectangle bounds, float padding, Texture body, Texture border,
            Texture bar, ImageButton exit) {
    ...
    public Window(String name, Font namefont, Color namecol, Rectangle bounds, float padding) {
        this(name, namefont, namecol, bounds, padding, getDefaultTextures().getTexture(1).convertToTexture(),
                getDefaultTextures().getTexture(2), getDefaultTextures().getTexture(0), new ImageButton(new Rectangle(
                        0, 0, 32, 32)));
    }
    
    public Window(String name, Font namefont, Color namecol, Rectangle bounds, float padding, Texture body,
            Texture border, Texture bar, ImageButton exit) {
        this(name, namefont, namecol, bounds, padding, body, border, bar.getHeight(), bar, exit);
    }
    
    public Window(String name, Font namefont, Color namecol, Rectangle bounds, float padding, Texture body,
            Texture border, float barheight, Texture bar, ImageButton exit) {
    ...
    public void render(Graphics g) {
        ...
        exit.bounds.translateTo(barbounds.getX() + exit.bounds.getWidth() / 6,

    ...
    public static Shader getShader(String src, int type) {
        ...
        try {
            if (type == Shader.FRAGMENT_SHADER) {
                vertShader = createVertexShader(readShader(Loader.streamFromClasspath("us/radiri/merc/gfx/default.vs")));
                fragShader = createFragmentShader(src);
            } else if (type == Shader.VERTEX_SHADER) {
                fragShader = createFragmentShader(readShader(Loader.streamFromClasspath("us/radiri/merc/gfx/default.fs")));

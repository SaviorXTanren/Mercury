...
public class Panel extends Component implements Container {
    ...
    public static SpriteSheet getDefaultTextures() {
        if (defaulttextures == null) {
            Texture sheet = Texture.loadTexture(Loader.streamFromClasspath("us/radiri/merc/gui/window.png"));
            SubTexture body = new SubTexture(sheet, 1, 41, 17, 57);
            SubTexture border = new SubTexture(sheet, 0, 40, 2, 512);
            SubTexture bar = new SubTexture(sheet, 0, 0, 512, 40);
            defaulttextures = SpriteSheet.loadSpriteSheet(sheet, body, border, bar);
        }

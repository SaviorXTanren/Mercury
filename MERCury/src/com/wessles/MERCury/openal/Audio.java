...
public class Audio implements Resource {
	...
	public static Audio loadAudio(String location, float srcx, float srcy, float velx, float vely) throws MERCuryException, IOException {
		...
		// Load WaveBufferData onto OpenAL
		AudioBufferData wbd = AudioBufferData.create(new BufferedInputStream(new FileInputStream(location)));
		AL10.alBufferData(buf.get(0), wbd.fmt, wbd.buf, wbd.smp);
		wbd.dispose();

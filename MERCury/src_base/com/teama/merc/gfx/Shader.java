...
public class Shader implements Resource
{
	...
	public static Shader getShader(String location, int type)
	{
		int vertShader = 0;
		int fragShader = 0;
		...
		try
		{
			...
			{
				vertShader = createVertexShader("res/gfx/shaders/default.vs");
				fragShader = createFragmentShader(location);
			}
			else if(type == Shader.VERTEX_SHADER)
			{
				fragShader = createFragmentShader("res/gfx/shaders/default.fs");
				vertShader = createVertexShader(location);
			}
		} catch (Exception exc)
		{
			exc.printStackTrace();
